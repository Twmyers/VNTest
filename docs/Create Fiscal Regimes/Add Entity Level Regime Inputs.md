...
# Add Entity-level Regime Inputs

An entity-level input creates an input on **Economics \| Interests &
Royalties** for all entities using the regime.
...
To add an entity-level regime input

1.  Beneath the input list, click **Add** and select **Entity-level
    Input**.
...
| Decimal places | Specify the number of decimal places to display on the input field. This sets the number of decimal places accepted from the user when entering an entity-level input. |
...
| Category | Optional. Production categories available for selection at the entity level. The input is displayed only for entities in this category. |
| Subcategory | Optional. Production categories available for selection at the entity level. The input is displayed only for entities in this subcategory. |
| Default Values | Optional. Enter the input default value(s). The values you enter are displayed in the input at the entity level, but they can be overridden. If you enter a default value, a null value is not allowed on the entity-level input. |

4.  **Optional**. Enter the input **Default** **Value**(s) on the right.
    The values you enter are displayed in the input at the entity level,
    but they can be overridden.
5.  Click **OK** or go to the next step, [Create a Calculated Variable](Add%20Variables%20to%20a%20Fiscal%20Regime/Create%20a%20Calculated%20Variable.md) or [Create a Trigger Variable](Add%20Variables%20to%20a%20Fiscal%20Regime/Create%20a%20Trigger%20Variable.md).
